...
//custom messages
void sendMessage();
void dropOffLocation(const std_msgs::UInt8::ConstPtr& message);
    ...
    if (currentMode == 2 || currentMode == 3) {
        ...
        case STATE_MACHINE_TRANSFORM: {
            ...
            if (targetCollected && !avoidingObstacle) {
                ...
                dropOffController.setCenterDist(hypot((centerLocation.x+=5) - (currentLocation.x+=5), (centerLocation.y+=5) - (currentLocation.y+=5)); //shifts eveything into the new drop off zones
...
void dropOffLocation(const std_msgs::UInt8::ConstPtr& message){
	int location = dropOffController.setDataLocations(centerLocation, currentLocation, timerTimeElapsed); // get whre we are at
...
void sendMessage(){ //used to send the location of tags and information to other robots
